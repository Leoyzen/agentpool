...
async def main() -> None:
    ...
    async with AgentPool(manifest) as pool:
        ...
        await scanner.run('List all Python files in "src/agentwolf/agent"')
        assert isinstance(scanner, Agent)
        # Use error checker as tool (this blocks until complete)
        scanner.register_worker(checker)
        prompt = 'Check types for all Python files in "src/agentwolf/agent"'
