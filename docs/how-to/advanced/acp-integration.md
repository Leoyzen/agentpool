...
```bash
uvx --python 3.13 agentwolf@latest serve-acp --help
```
  ...
  "agent_servers": {
    "AgentPool": {
      ...
      "args": [
        "--python",
        "3.13",
        "agentwolf[coding]@latest",
        "serve-acp",
        "https://raw.githubusercontent.com/Million-mo/agentwolf/refs/heads/main/docs/examples/create_docs/config.yml"
      ],
      "env": {
...
1. Set the command to `agentwolf` (or `uvx agentwolf@latest`)
...
When you run `agentwolf serve-acp`, it becomes an **ACP server** that IDEs can connect to:

```mermaid
graph LR
    A["IDE (Zed)<br/>ACP Client"] <-->|ACP Protocol| B["agentwolf<br/>ACP Server<br/>(serve-acp command)"]
```
...
```mermaid
graph LR
    A["agentwolf<br/>ACP Client<br/>(external agent)"] <-->|ACP Protocol| B["Claude Code<br/>ACP Server<br/>(subprocess)"]
```
...
graph TB
    ...
    LLM["agentwolf<br/>Server ↔ Pool ↔ Client"]
...
For MCP-capable ACP agents (Claude, Gemini, Auggie, Kimi), agentwolf can automatically expose internal toolsets via an in-process MCP server bridge. This allows external agents to use powerful internal capabilities like subagent delegation, agent management, and custom tools.
...
```bash
agentwolf serve-acp https://example.com/config.yml
```
