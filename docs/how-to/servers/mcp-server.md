...
agentwolf serve-mcp config.yml --toolset file_access --toolset search
```
...
```bash
# Start MCP server
agentwolf serve-mcp tools.yml --transport sse --port 3001
```
...
```bash
agentwolf serve-mcp --config agent.yml
```
...
```bash
agentwolf serve-mcp config.yml --transport stdio
```
