...
- Add agent files with `agentwolf add <name> <path>`
- Set the active file with `agentwolf set <name>`
- List agents from the active config with `agentwolf list`
...
AgentPool automatically discovers config files. Just create `agentwolf.yml` in your project:
...
1. Add and activate an agent configuration:
   ...
   ```bash
   agentwolf add myconfig agents.yml
   agentwolf set myconfig
   ```
...
3. Run a prompt with an agent:
   ...
   ```bash
   agentwolf run analyzer "Analyze this text"
   ```
...
agentwolf run myagent "Analyze this"

# Run a team
agentwolf run myteam "Process this"

# Show detailed output with costs
agentwolf run myagent "Hello" --detail full --costs
```
...
agentwolf task docs write_api_docs

# Execute with additional prompt
agentwolf task docs write_api_docs --prompt "Include code examples"
```
...
agentwolf serve-mcp config.yml --transport sse --port 3001
