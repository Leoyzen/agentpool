...
This RFC proposes updating agentwolf's `/global/event` SSE output to conform to the new `GlobalEvent` protocol, ensuring compatibility with OpenCode v1.4.4+ while maintaining backward compatibility with v1.4.3 clients via the unchanged `/event` endpoint.
...
- **Critical**: All users on OpenCode v1.4.4+ cannot use agentwolf's `serve-opencode` at all
- **Blocking**: agentwolf is effectively incompatible with the current OpenCode release
...
3. Adding workspace-level isolation (multi-workspace) — agentwolf serves a single directory; `workspace` field omitted
...
- `workspace` field is intentionally omitted — agentwolf doesn't have a workspace concept
...
|------|------------|--------|------------|
| `workspace` field semantics unclear | Medium | Low | Omit `workspace` entirely; document that agentwolf doesn't support workspaces |
...
class GlobalEventFactory:
    ...
    def __init__(self, state: ServerState) -> None:
        ...
        # workspace is intentionally omitted (None) — agentwolf
...
3. **Should agentwolf support `directory: "global"` for cross-project events?**
