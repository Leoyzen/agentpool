...
AgentPool's session orchestration lives in `src/agentwolf/orchestrator/core.py`, combining `SessionController` (session CRUD) and `TurnRunner` (turn execution + manual queue management). Key components:
