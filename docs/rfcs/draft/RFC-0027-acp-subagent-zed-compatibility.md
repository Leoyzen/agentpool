...
- `packages/agentwolf/src/acp/schema/base.py` — `AnnotatedObject` 基类，`field_meta` 字段（序列化为 `_meta`）
- `packages/agentwolf/src/acp/schema/session_updates.py` — `ToolCallStart`/`ToolCallProgress` 定义

### AgentPool 源码

- `packages/agentwolf/src/agentwolf_server/acp_server/event_converter.py` — 核心 ACP 事件转换器
- `packages/agentwolf/src/agentwolf_server/acp_server/session.py` — ACP session 管理
- `packages/agentwolf/src/agentwolf_server/acp_server/session_manager.py` — Session 生命周期管理
- `packages/agentwolf/src/agentwolf_server/acp_server/server.py` — ACP server 入口
- `packages/agentwolf_toolsets/builtin/subagent_tools.py` — 发射 SubAgentEvent 和 SpawnSessionStart
- `packages/agentwolf/src/agentwolf/agents/events/events.py` — 事件定义：SubAgentEvent（line 614）、SpawnSessionStart（line 646）
