...
| Package | Role |
|---------|------|
| `agentwolf/` | Core agent framework (agents, delegation, messaging, tools, models, prompts, storage, hooks, observability) |
| `agentwolf_config/` | YAML schema definitions (config models, separated for clean imports) |
| `agentwolf_server/` | Protocol servers (ACP, OpenCode, AG-UI, OpenAI API, MCP) |
| `agentwolf_toolsets/` | Reusable toolset implementations (builtin, MCP discovery, specialized) |
| `agentwolf_storage/` | Storage providers (SQL, Zed, Claude, OpenCode) |
| `agentwolf_cli/` | Command-line interface |
| `agentwolf_commands/` | Command implementations |
| `agentwolf_prompts/` | Prompt templates |
...
4. **Package naming**: Should v1.0 use `agentwolf_*` or keep `agentwolf_*` package names? (Recommendation: keep `agentwolf_*` for backward compatibility; `agentwolf` is an internal codename.)
