...
### agentwolf's Current Misalignment
...
- **pydantic-ai misalignment**: agentwolf's agent model diverges from the framework it's built on
...
**After refactor, `self.conversation` is REMOVED from `BaseAgent`** (hard cut — no dual-path deprecation). All callers must pass `message_history=` explicitly to `run_stream()`. This includes CLI usage (`agentwolf run`), which must manage its own message history list.
...
2. **Backward compatibility for `self.conversation`**: Should `BaseAgent.conversation` still exist as a fallback for non-server callers?
    - Context: Direct agent usage (e.g., `agentwolf run`) doesn't have a server managing message history.
...
- `packages/agentwolf/src/agentwolf/agents/base_agent.py` — `BaseAgent`, session-scoped instance variables
- `packages/agentwolf/src/agentwolf/agents/agent.py` — `Agent`, `_run_stream_once()`, `self.conversation` references
- `packages/agentwolf/src/agentwolf/running/run_context.py` — `AgentRunContext`, per-run isolation
