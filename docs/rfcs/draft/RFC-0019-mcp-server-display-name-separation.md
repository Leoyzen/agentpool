...
1. **MCPManager initialization** (`src/agentwolf/delegation/pool.py:149`):
   ```python
   self.mcp = MCPManager(name="pool_mcp", servers=servers, owner="pool")
   ```

2. **Provider name construction** (`src/agentwolf/mcp_server/manager.py:137`):
   ```python
   name=f"{self.name}_{config.client_id}"
   ```

3. **Client ID generation** (`src/agentwolf_config/mcp_server.py`):
...
#### 1. Config Layer (`src/agentwolf_config/mcp_server.py`)
...
#### 2. Manager Layer (`src/agentwolf/mcp_server/manager.py`)
...
#### 3. API Layer (`src/agentwolf_server/opencode_server/routes/agent_routes.py`)
