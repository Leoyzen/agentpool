...
AgentPool has a `SessionManager` class (`src/agentwolf/sessions/manager.py`) with a `create_child_session()` method that:
...
| **SessionManager** | Central class for session CRUD and child session creation (`src/agentwolf/sessions/manager.py`) |
...
```python
# src/agentwolf_toolsets/builtin/subagent_tools.py
...
```python
# src/agentwolf/delegation/team.py
...
```python
# src/agentwolf/delegation/teamrun.py
...
```python
# src/agentwolf_server/acp_server/session_manager.py
...
```python
# src/agentwolf_server/opencode_server/state.py
...
```python
# src/agentwolf/messaging/messagenode.py
...
class MessageNode:
    ...
    def agent_type(self) -> str:
        ...
        from agentwolf.delegation.team import Team
        from agentwolf.delegation.teamrun import TeamRun
...
```python
# src/agentwolf/agents/helpers.py
...
**Pre-existing codebase bug**: `team.py:32` and `teamrun.py:28` import `SubAgentType` from `agentwolf.agents.events.events` inside `TYPE_CHECKING` blocks, but `SubAgentType` does **not exist** in that module. This is a type-checking error that's silently ignored at runtime. The new `SourceType` type alias and `get_source_type()` function in `helpers.py` replace this broken import. **Implementation action**: Update the `TYPE_CHECKING` imports in `team.py` and `teamrun.py` to use `from agentwolf.agents.helpers import SourceType` instead of the broken `SubAgentType` import.
...
```python
# src/agentwolf/agents/base_agent.py or constants
...
`DelegationDepthError` is defined in `src/agentwolf/agents/exceptions.py` alongside other agent-specific exceptions. `MAX_DELEGATION_DEPTH` is a module-level constant in the same file.

```python
# src/agentwolf/agents/exceptions.py
...
async def test_messagenode_agent_type_and_source_type():
    ...
    from agentwolf.agents.helpers import get_source_type
...
async def test_get_source_type_compatible_with_source_type_literal():
    ...
    from agentwolf.agents.helpers import get_source_type, SourceType
...
def test_messagenode_agent_type_no_circular_import():
    ...
    import importlib
    import agentwolf.messaging.messagenode

    # Force re-import to detect circular dependency
    importlib.reload(agentwolf.messaging.messagenode)
...
| 5 | `DelegationDepthError` referenced but undefined — no module location specified | Oracle SHOULD | ✅ Added definition in `src/agentwolf/agents/exceptions.py` with `MAX_DELEGATION_DEPTH` constant |
...
- `src/agentwolf/sessions/manager.py` — `SessionManager.create_child_session()`
- `src/agentwolf/sessions/store.py` — `SessionStore` protocol
- `src/agentwolf/sessions/models.py` — `SessionData` model
- `src/agentwolf_toolsets/builtin/subagent_tools.py` — SubagentTools
- `src/agentwolf_toolsets/builtin/workers.py` — WorkersTools
- `src/agentwolf/delegation/team.py` — Team (parallel)
- `src/agentwolf/delegation/teamrun.py` — TeamRun (sequential)
- `src/agentwolf/agents/context.py` — `AgentContext`
- `src/agentwolf/agents/events/events.py` — `SpawnSessionStart`, `SubAgentEvent`
- `src/agentwolf_server/opencode_server/state.py` — `ServerState.ensure_session()`
- `src/agentwolf_server/opencode_server/event_processor.py` — `EventProcessor`
- `src/agentwolf_server/acp_server/session_manager.py` — `ACPSessionManager`
