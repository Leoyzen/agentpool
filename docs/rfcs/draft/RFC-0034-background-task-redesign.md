...
4. **No upstream standard**: Each downstream project reinvents background task patterns; agentwolf lacks a unified abstraction.
...
- **Unified infrastructure**: `BackgroundTaskManager` and `TaskHandle` are core agentwolf primitives.
...
| **Provider integration** | `xeno_agent/agentwolf/resource_providers/background_task_provider.py` | Tool definitions (`task`, `background_output`, `background_cancel`) |
...
| **SessionPool** | agentwolf's session orchestration layer (`SessionPool` + `SessionController`) |
...
- **RFC-0001 (agentwolf)**: Workers and Teams session management establishes `SpawnSessionStart` patterns that background tasks must emit consistently.
...
4. **No upstream abstraction**: Every downstream project that wants background tasks must copy xeno-agent's implementation. There is no agentwolf-native primitive.
...
1. **Upstream core infrastructure**: `BackgroundTaskManager`, `TaskHandle`, and `TaskStatus` become agentwolf core primitives.
...
3. **Not**: Distributed task execution across multiple agentwolf instances.
...
- [ ] `TaskHandle` is importable from `agentwolf.tasks`.
- [ ] `BackgroundTaskManager` is importable from `agentwolf.tasks`.
- [ ] `session_end_policy` is configurable per `AgentPool` or per session.
- [ ] xeno-agent's `BackgroundTaskProvider` delegates to agentwolf core.
...
**Decision: C — Hybrid approach: agentwolf core infrastructure + xeno-agent specific Provider**

**Rationale**:

The core lifecycle manager (`BackgroundTaskManager`), types (`TaskHandle`, `BackgroundTask`), and session integration belong in agentwolf so all downstream projects benefit. However, the specific tool schemas, prompt formatting, and xeno-agent-specific behaviors (e.g., `load_skills` injection, XML prompt formatting) remain in xeno-agent's `BackgroundTaskProvider`.

**Scope split**:

| Component | agentwolf (core) | xeno-agent (provider) |
|-----------|-----------------|----------------------|
| `TaskHandle` | ✅ | ❌ |
...
- **Pro**: agentwolf can integrate background tasks with `SessionPool` natively.
- **Con**: Slightly more complex import graph; xeno-agent depends on agentwolf tasks.
...
```python
from agentwolf.tasks import TaskHandle
...
A single hardcoded behavior cannot satisfy all use cases. Industrial diagnostics (`xeno-agent`) often needs `notify` so the fault expert receives completion prompts. Batch processing jobs need `keep`. Default agentwolf behavior should be `cancel` for safety.
...
- `xeno-agent` provider: `BackgroundTaskProvider` rewritten to delegate to agentwolf core.
...
#### 7.2.1 `agentwolf.tasks` Module (New)

```python
# agentwolf/tasks/__init__.py
...
#### 7.2.2 `BackgroundTaskManager` (agentwolf core)

```python
# agentwolf/tasks/manager.py
from __future__ import annotations
...
class BackgroundTaskManager:
    ...
    This is the agentwolf-core equivalent of xeno-agent's
...
```python
# xeno_agent/agentwolf/resource_providers/background_task_provider.py
...
agents:
  fault_expert:
    ...
    tools:
      - type: custom
        import_path: xeno_agent.agentwolf.resource_providers.background_task_provider.XenoBackgroundTaskProvider
        enabled_tools:
          - run_background_task
          - task_status
          - task_result
...
```python
# agentwolf/sessions/controller.py

from agentwolf.tasks import BackgroundTaskManager
...
### 11.2 agentwolf-to-SEP-1686 Mapping

| SEP-1686 Concept | agentwolf Equivalent | Notes |
|-----------------|----------------------|-------|
...
When agentwolf implements an MCP server with SEP-1686 support:

```python
# Future: agentwolf MCP server
from agentwolf.tasks import BackgroundTaskManager
...
- [ ] **Phase 0**: Create agentwolf `tasks` module with `TaskHandle`, `BackgroundTask`, `BackgroundTaskManager`.
- [ ] **Phase 0**: Port `BackgroundTaskManager` from xeno-agent to agentwolf, adding `cancel_and_drain` compliance.
...
### Phase 0: agentwolf Core (Week 1)

**Deliverables**:
- `agentwolf/tasks/types.py` — `TaskStatus`, `BackgroundTask`, `TaskHandle`
- `agentwolf/tasks/manager.py` — `BackgroundTaskManager` with `cancel_and_drain`
- `agentwolf/tasks/__init__.py` — Public exports
- Unit tests for core manager (semaphore, timeout, cancellation, cleanup)

**Files Modified**:
- `src/agentwolf/tasks/` (new directory)
- `src/agentwolf/sessions/controller.py` (session_end_policy hook)
- `src/agentwolf/sessions/config.py` (session_end_policy field)

**Rollback**: Delete `src/agentwolf/tasks/` directory.
...
**Files Modified**:
- `src/agentwolf/sessions/config.py`
- `src/agentwolf/sessions/controller.py`
- `src/agentwolf/tasks/manager.py`
...
**Files Modified**:
- `src/xeno_agent/agentwolf/resource_providers/background_task_provider.py`
...
**Files Modified**:
- `tests/agentwolf/resource_providers/test_background_task_*.py`

**Rollback**: Restore from git.

### Dependencies

- agentwolf Phase 0 must complete before xeno-agent Phase 2.
- agentwolf Phase 1 must complete before xeno-agent test additions for `session_end_policy`.
...
5. **Should agentwolf expose a `@background_task` decorator for tool functions?**
...
8. **Should `list_tasks()` be a core agentwolf tool or a xeno-agent provider tool?**
...
1. **Why not Option A (all in agentwolf)?** Would force xeno-agent's XML formatting and skill-loading into core, creating inappropriate coupling.
...
- [ ] agentwolf core tests demonstrate 100% manager coverage
...
- [ ] Documentation updated (agentwolf tasks module, xeno-agent migration guide)
...
2. **RFC-0021 (agentwolf)** — Agent Concurrent Execution Safety
   Location: `packages/agentwolf/docs/rfcs/implemented/RFC-0021-agent-concurrent-execution-safety.md`

3. **BackgroundTaskProvider Implementation**
   Location: `packages/xeno-agent/src/xeno_agent/agentwolf/resource_providers/background_task_provider.py`
...
| xeno-agent BackgroundTaskProvider | `packages/xeno-agent/src/xeno_agent/agentwolf/resource_providers/background_task_provider.py` |
| Pydantic-AI cancel_and_drain | `packages/pydantic-ai/pydantic_ai_slim/pydantic_ai/_utils.py:223-240` |
| AgentPool SessionController | `packages/agentwolf/src/agentwolf/sessions/controller.py` |
| AgentPool SessionConfig | `packages/agentwolf/src/agentwolf/sessions/config.py` |
