...
```python
# File: src/agentwolf/simulation/input_provider.py
class SimulationInputProvider(InputProvider):
...
```python
# File: src/agentwolf/simulation/events.py
from dataclasses import dataclass
from agentwolf.models import CustomEvent
...
```python
# File: src/agentwolf/simulation/tool_provider.py
class SimulationToolProvider(ResourceProvider):
...
2. **CLI Commands**
   - `agentwolf simulate --sim-agent X --target Y`
...
# View trajectories
agentwolf history view --agent diagnosis --session <session_id>

# Evaluate results
agentwolf simulate-eval results/ --criteria criteria.yml
```
