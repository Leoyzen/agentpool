...
- **Risk**: Teams sharing an agentwolf server see each other's task lists. This is a functional correctness issue, not just a UX concern.
...
async def on_acp_todo_change(t: TodoTracker) -> None:
    ...
    from agentwolf.utils.todos import PlanEntry
...
- `src/agentwolf/utils/todos.py` — `TodoTracker`, `TodoEntry`, `PlanEntry`
- `src/agentwolf/delegation/pool.py` — `AgentPool.todos` (global instance)
- `src/agentwolf/resource_providers/plan_provider.py` — `PlanProvider` (tool implementation)
- `src/agentwolf_server/opencode_server/server.py` — Global `on_todo_change` callback
- `src/agentwolf_server/opencode_server/state.py` — `ServerState.todos` (in-memory cache)
- `src/agentwolf_server/opencode_server/routes/session_routes.py` — `get_session_todos()` route
- `src/agentwolf_server/acp_server/event_converter.py` — `PlanUpdateEvent` → `AgentPlanUpdate`
- `src/agentwolf/agents/acp_agent/client_handler.py` — Unresolved `AgentPlanUpdate` TODO
- `src/agentwolf/agents/codex_agent/codex_agent.py` — `pool.todos.replace_all()` sync
