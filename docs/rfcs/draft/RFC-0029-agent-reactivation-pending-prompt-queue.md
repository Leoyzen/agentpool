...
- **Event loop affinity**: `asyncio.Queue` and `asyncio.Event` are not thread-safe. All callers must be in the same event loop. This matches current agentwolf architecture where all agent operations are asyncio-based.
...
- `src/agentwolf/agents/base_agent.py` — `inject_prompt()`, `queue_prompt()`, `run_stream()`
- `src/agentwolf/agents/prompt_injection.py` — `PromptInjectionManager`
- `src/agentwolf/agents/native_agent/hook_manager.py` — injection consumption in post-tool hooks
- `src/agentwolf_server/opencode_server/routes/message_routes.py` — `_run_async_prompt_queue()`
