...
**Skills System** (`src/agentwolf/skills/`):
...
**OpenCode Protocol** (`src/agentwolf_server/opencode_server/`):
...
**AG-UI Protocol** (`src/agentwolf_server/agui_server/`):
...
```python
# src/agentwolf/skills/command.py
...
```python
# src/agentwolf/skills/command_registry.py
...
from agentwolf.utils.baseregistry import BaseRegistry
...
**OpenCode Bridge** (`src/agentwolf_server/opencode_server/skill_bridge.py`):
...
if TYPE_CHECKING:
    from agentwolf.agents.opencode_agent import OpenCodeAgent
    from agentwolf.skills.command import SkillCommand
...
**ACP Bridge** (`src/agentwolf_server/acp_server/commands/skill_commands.py`):
...
**AG-UI Bridge** (`src/agentwolf_server/agui_server/skill_tools.py`):
...
```python
# src/agentwolf_config/skill_commands.py
...
async def _setup_skill_commands(self) -> None:
    ...
    from agentwolf.skills.command_registry import SkillCommandRegistry
...
```python
# In agentwolf_server/opencode_server/server.py

async def _setup_skill_commands(self) -> None:
    """Setup skill command bridge with OpenCode's native slashed system."""
    from agentwolf.skills.command_registry import SkillCommandRegistry
    from agentwolf_server.opencode_server.skill_bridge import OpenCodeSkillBridge
...
```python
# In agentwolf_server/acp_server/session.py or agent.py

async def _get_capabilities(self) -> AgentCapabilities:
    """Get agent capabilities including skill commands."""
    from agentwolf_server.acp_server.commands.skill_commands import ACPSkillBridge
...
1. **Create SkillCommand dataclass** (`src/agentwolf/skills/command.py`)
   - Define protocol-agnostic command representation
   - Add validation methods

2. **Create SkillCommandRegistry** (`src/agentwolf/skills/command_registry.py`)
...
3. **Integrate with AgentPool** (`src/agentwolf/delegation/pool.py`)
...
1. **Create ACPSkillBridge** (`src/agentwolf_server/acp_server/commands/skill_commands.py`)
...
3. **Integrate with ACPServer** (`src/agentwolf_server/acp_server/server.py`)
...
1. **Create AGUISkillBridge** (`src/agentwolf_server/agui_server/skill_tools.py`)
...
1. **Create OpenCodeSkillBridge** (`src/agentwolf_server/opencode_server/skill_bridge.py`)
...
3. **Integrate with OpenCodeServer** (`src/agentwolf_server/opencode_server/server.py`)
...
**Schema Addition**:
```python
# agentwolf_config/skill_commands.py
