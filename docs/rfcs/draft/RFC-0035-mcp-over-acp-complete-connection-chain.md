...
对于 stdio/SSE/HTTP 传输的 MCP server，agentwolf 已经有一套成熟的连接链：
...
关键组件：
- **`MCPResourceProvider`**（`agentwolf/resource_providers/mcp_provider.py`）：MCP server 的 ResourceProvider 包装，管理工具/提示/资源的生命周期
- **`MCPClient`**（`agentwolf/mcp_server/client.py`）：fastmcp Client 的包装，处理连接、重连、消息路由
...
- **`AcpMcpTransport`**（`agentwolf_server/acp_server/acp_mcp_transport.py`）：实现了 `fastmcp.ClientTransport` 接口，将内存流包装为 fastmcp 可用的传输层
...
- **没有工具注册**：`tools/list` 的结果需要转换为 agentwolf 的 `Tool` 对象并注册到 Agent 运行时
...
```python
# agentwolf/mcp_server/client.py
...
```python
# agentwolf_server/acp_server/acp_mcp_transport.py
...
```python
# agentwolf_server/acp_server/session.py
...
| 文件 | 说明 |
|------|------|
| `agentwolf/mcp_server/client.py` | MCPClient，需修改 `_get_client()` |
| `agentwolf/resource_providers/mcp_provider.py` | MCPResourceProvider，工具注册 |
| `agentwolf_server/acp_server/acp_mcp_transport.py` | AcpMcpTransport，需支持延迟初始化 |
| `agentwolf_server/acp_server/acp_agent.py` | connect_acp_mcp_server() |
| `agentwolf_server/acp_server/session.py` | initialize_mcp_servers() |
| `agentwolf_server/acp_server/acp_mcp_manager.py` | AcpMcpConnectionManager |
