...
> **Phase 3 of the Multi-Session Isolation Roadmap.** Depends on [RFC-0024](./RFC-0024-agent-stateless-refactor.md) (Phase 2). This is the convergence point that aligns agentwolf's server architecture with pydantic-ai's stateless agent model.
...
Phase 3 makes agentwolf's server match this model exactly: one `BaseAgent` instance, many `run_stream()` calls, each with its own `message_history` and `input_provider`.
...
- `packages/agentwolf/src/agentwolf_server/opencode_server/state.py` — `ServerState`, per-session agent registry
- `packages/agentwolf/src/agentwolf_server/opencode_server/routes/session_routes.py` — Session CRUD
- `packages/agentwolf/src/agentwolf_server/opencode_server/routes/message_routes.py` — Message handling
- `packages/agentwolf/src/agentwolf/agents/base_agent.py` — `BaseAgent` (stateless after Phase 2)
