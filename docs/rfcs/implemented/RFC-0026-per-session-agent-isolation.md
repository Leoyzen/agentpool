...
When multiple OpenCode clients connect to the same `agentwolf serve-opencode` server, only the first can process messages. All others are blocked by a global `asyncio.Lock` held for the entire duration of LLM inference (10–120 seconds per response).
...
**Impact**: Teams cannot share an agentwolf server. Each user must run their own instance.
...
- `packages/agentwolf/src/agentwolf_server/opencode_server/state.py` — `ServerState`, `agent_lock`, `bind_agent_to_session()`
...
- `packages/agentwolf/src/agentwolf/agents/base_agent.py` — `BaseAgent`, shared mutable state
