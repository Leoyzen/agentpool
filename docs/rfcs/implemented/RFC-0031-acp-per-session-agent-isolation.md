...
When multiple ACP clients connect to the same `agentwolf serve-acp` server, all sessions share a single `BaseAgent` instance. This causes:
...
- **Cost**: Users must run separate `agentwolf serve-acp` processes per client, defeating the purpose of the WebSocket/multi-client transport.
...
- `src/agentwolf_server/acp_server/server.py` — `ACPServer`, pool lifecycle
- `src/agentwolf_server/acp_server/acp_agent.py` — `AgentPoolACPAgent`, protocol methods
- `src/agentwolf_server/acp_server/session_manager.py` — `ACPSessionManager`, session tracking
- `src/agentwolf_server/acp_server/session.py` — `ACPSession`, prompt processing
- `src/agentwolf_server/opencode_server/state.py` — Reference: OpenCode's `_session_agents`, `get_or_create_agent()`
