...
本 RFC 提议在 agentwolf 中增加对 MCP-over-ACP 传输协议的支持，允许 ACP 客户端通过现有 ACP 连接注入 MCP 工具服务，而无需单独的 stdio 进程或 HTTP 端口。实现后，agentwolf 可作为 ACP Agent 向客户端声明 `mcpCapabilities.acp: true`，并在 session 初始化阶段**主动**通过 `mcp/connect` 向客户端请求建立连接（Agent→Client），通过 `mcp/message` 双向转发工具调用，在 session 结束时通过 `mcp/disconnect` 通知客户端清理（Agent→Client），完成完整的 MCP 工具调用生命周期。

该特性能够显著扩大 agentwolf 的适用场景：客户端可注入项目感知工具（如代码感知的搜索工具）、沙箱环境（如 WASM 运行时）也能提供 MCP 工具，同时消除了通过"旁路"传输带来的沙箱逃逸风险。
...
agentwolf 当前通过 ACP 协议对外提供 Agent 能力。在 `session/new` 和 `session/load` 请求中，客户端可传入 MCP 服务器配置（`mcp_servers`），agentwolf 支持三种 MCP 传输类型：
...
- [RFC-0030](../draft/RFC-0030-acp-streamable-http-websocket-transport.md) 扩展了 agentwolf 对 Streamable HTTP / WebSocket 的 ACP 传输层支持，为本 RFC 奠定了传输基础设施基础。
...
| Term | Definition |
|------|------------|
| ACP | Agent Client Protocol，agentwolf 使用的 Agent-客户端双向通信协议 |
...
- agentwolf 的 `convert_acp_mcp_server_to_config()` 在遇到未知 MCP server 类型时直接 `assert_never`（即崩溃），无任何扩展入口
- `McpCapabilities` 中无 `acp` 字段，无法向客户端声明支持能力

### Impact of Inaction

- **功能缺失**：无法支持 ACP 客户端的 MCP 工具注入场景（如 Zed 插件、WASM 沙箱工具）
- **生态兼容性**：ACP 参考实现（Rust SDK sacp-conductor）已实现 Bridging，agentwolf 无法与其互操作
...
1. agentwolf 作为 ACP Agent 在 `InitializeResponse` 中声明 `mcpCapabilities.acp: true`
...
7. **双向配置转换**：同步更新正向转换（`converters.py`）和反向转换（`acp_converters.py`），确保 agentwolf 作为 ACP 客户端时代码不崩溃

### Non-Goals (Out of Scope)

1. **Bridging（可选，后续 RFC）**：将 ACP-transport MCP Server 透明转为 stdio/HTTP shim 不在本 RFC 范围内
2. **客户端侧实现**：本 RFC 仅覆盖 agentwolf（Agent 侧）实现，不涉及客户端 SDK 变更
...
- [ ] 客户端传入 `type: "acp"` MCP Server 后，agentwolf 在 session 初始化阶段**主动**发送 `mcp/connect`（Agent→Client），客户端返回 `connectionId`
...
在 agentwolf 的 ACP server 层直接实现完整的 MCP-over-ACP 协议处理器：
...
不修改 agentwolf 核心，而是在 agentwolf 外层增加一个 Conductor/Proxy 进程：对外声明支持 `mcpCapabilities.acp`，接收到 ACP-transport MCP Server 声明后，生成一个本地 stdio shim 进程，将 ACP channel 消息转换为 stdio MCP 消息，再传给 agentwolf（以普通 stdio MCP Server 形式传入）。
...
等待 ACP 官方 Rust SDK（sacp-conductor）稳定后，通过集成官方 bridge 实现支持，agentwolf 自身不做任何改动。
...
┌──────────────────────────▼──────────────────────────────────────┐
│  agentwolf ACP Server (AgentPoolACPAgent / acp_agent.py)        │
│                                                                  │
  │  ┌─────────────────────────────────────┐                        │
...
新增 `agentwolf_server/acp_server/acp_mcp_manager.py`：
...
**新增 `agentwolf_server/acp_server/acp_mcp_transport.py`**：
...
在 `agentwolf_config/mcp_server.py` 中新增：
...
在 `agentwolf/mcp_server/provider.py` 中更新 `transport_type`：
...
|--------|--------|------------|------------|
| 客户端注入恶意 MCP 工具 | High | Medium | 与现有 MCP 信任模型一致：agentwolf 不对 MCP 工具内容进行额外验证，工具调用权限由用户在会话级别授权 |
| `connectionId` 伪造/碰撞 | Medium | Low | `connectionId` 由客户端生成，Agent 仅验证其是否存在于本地连接表；使用完整 `uuid4().hex`（128-bit 熵），不可预测 |
| `mcp/message` 参数注入 | Low | Low | 透明转发，agentwolf 不解析工具参数内容；安全责任由 MCP Server（客户端侧）承担 |
...
- **Scope**：
  ...
  - 更新 `MCPServerConfig` union（`agentwolf_config/mcp_server.py`）
...
2. **pydantic-ai MCP transport 接入点**
   - **Decision**：不直接对接 pydantic-ai，而是实现 `fastmcp.ClientTransport`，通过 `fastmcp.Client` 接入 agentwolf 现有的 `MCPClient`
   - **Rationale**：agentwolf 实际使用 fastmcp（而非 pydantic-ai 的 MCP 抽象）管理 MCP 连接。`ClientTransport` 是正确的扩展点
...
2. **fastmcp pinned 版本是否支持自定义 ClientTransport**
   - Context：需确认 agentwolf 当前锁定的 fastmcp 版本是否暴露 `ClientTransport` 接口
   - Owner: 本 RFC 实现者
...
| `AcpMcpServer` handler | `agentwolf_server/acp_server/converters.py` | 转换逻辑（正向） |
| `AcpMcpServer` reverse handler | `agentwolf_server/acp_server/acp_converters.py` | 转换逻辑（反向） |
| `AcpMCPServerConfig` | `agentwolf_config/mcp_server.py` | Config 类 |
| `AcpMcpConnectionManager` | `agentwolf_server/acp_server/acp_mcp_manager.py` | 核心逻辑 |
| `AcpMcpTransport` | `agentwolf_server/acp_server/acp_mcp_transport.py` | fastmcp Transport |
...
| 文件 | 函数/属性 | 操作 |
|------|----------|------|
| `agentwolf_server/acp_server/converters.py` | `convert_acp_mcp_server_to_config()` | 新增 `case AcpMcpServer()` |
| `agentwolf_server/acp_server/acp_converters.py` | `mcp_config_to_acp()` | 新增 `case AcpMCPServerConfig()` |
| `agentwolf/mcp_server/provider.py` | `transport_type` property | 新增 `"acp"` 到 return `Literal` |
| `agentwolf/mcp_server/client.py` | `MCPClient._get_client()` | 新增 `case AcpMCPServerConfig()` |
| `agentwolf_config/mcp_server.py` | `MCPServerConfig` union | 新增 `AcpMCPServerConfig` |
| `agentwolf_config/mcp_server.py` | `parse_mcp_servers_json()` | 新增 `"acp"` transport 分支 |
