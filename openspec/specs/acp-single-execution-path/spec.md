...
There SHALL be exactly one `RunFailedEvent` type, defined in `agentwolf/agents/events/events.py` and published via `RunHandle.fail()` to the EventBus. The signal-based `BaseAgent.RunFailedEvent` inner class SHALL be removed.
