...
The `agentwolf serve-acp` CLI command SHALL accept `--port` and `--host` optional flags. When provided, these flags SHALL create a `StreamableHTTPTransport` with the specified values. When neither flag is provided, the CLI SHALL default to stdio transport.

#### Scenario: CLI with --port flag
- **WHEN** the user runs `agentwolf serve-acp config.yml --port 8080`
...
- **WHEN** the user runs `agentwolf serve-acp config.yml --host 0.0.0.0 --port 9000`
- **THEN** the CLI SHALL create `StreamableHTTPTransport(host="0.0.0.0", port=9000)`

#### Scenario: CLI without transport flags
- **WHEN** the user runs `agentwolf serve-acp config.yml`
