...
- `content_blocks: list[dict[str, Any]] | None` SHALL default to `None`. When provided, it carries structured content in ACP `ContentBlock[]` shape. When `None`, `content` is the plain-text representation.
