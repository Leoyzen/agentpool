...
7. **Wire up in the ACP server** (if needed):
   - `src/agentwolf_server/acp_server/acp_agent.py` — implement new methods, advertise new capabilities
   - `src/agentwolf_server/acp_server/event_converter.py` — emit new session update types
   - `src/agentwolf_server/acp_server/session.py` — store new state
...
- `client/` implementations (`DefaultACPClient`, `HeadlessACPClient`, `NoOpClient`) — agentwolf-specific
