...
def serve(ctx, *args: str):
    ...
    if reorder:
        ...
        # Create a temp structure to match zensical's URL scheme: /agentwolf/
        import tempfile

        with tempfile.TemporaryDirectory() as tmpdir:
            tmp_path = Path(tmpdir)
            (tmp_path / "agentwolf").symlink_to(Path("site").resolve())
            print(f"Serving at http://localhost:{port}/agentwolf/")
...
def _get_lint_targets(filepath: str | None) -> tuple[str, str, str | None]:
    ...
    if filepath is None:
        return (
            ".",
            "src/",
            "src/agentwolf/config_resources/*.yml docs/examples/**/config.yml",
        )
...
def lint(ctx, filepath: str | None = None):  # noqa: D417
    ...
    if jsonschema_files:
        ...
        ctx.run(
            ...
            "src/agentwolf/config_resources/*.yml "
...
def smoke_test(ctx, timeout: int = 10):  # noqa: D417
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        result = subprocess.run(  # noqa: PLW1510
            ["timeout", str(timeout), "uvx", "--from", str(wheel), "agentwolf", "serve-acp"],
            capture_output=True,
            text=True,
        )
...
def opencode_server(ctx, *args: str):
    ...
    """
    args_str = " " + " ".join(args) if args else ""
    ctx.run(f"uv run agentwolf serve-opencode{args_str}")
