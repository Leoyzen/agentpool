...
| **L4** | Subprocess E2E | `@pytest.mark.e2e` | Real `agentwolf serve-*` process + protocol client | Slow (5-30s) | Optional | L4a: ✅ PR smoke, L4b: ❌ Nightly | Server startup, stdio transport, process lifecycle, real I/O timing |
...
```python
@pytest.mark.skip(reason="mcp/connect client-side not implemented in agentwolf ACP server yet")
...
| `subprocess_server` | function | Spawned `agentwolf serve-*` process (cached by serve_command; use `--no-server-cache` to bypass) |
...
The `subprocess_server` fixture uses a **session-scoped cache** to reuse `agentwolf serve-*` processes across tests with the same `serve_command`. This reduces ~100+ subprocess spawns to ~4-5, cutting L4a e2e time from ~14min to ~4-5min.
