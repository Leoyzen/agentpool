...
These tests verify:
1. /agent endpoint returns mode="primary" for all agentwolf agents (correct)
...
from agentwolf_server.opencode_server.converters import chat_message_to_opencode
...
async def test_agent_endpoint_mode_is_primary_for_all_agents() -> None:
    """GET /agent should return mode='primary' for all agentwolf agents.

    AgentMode is Literal['subagent', 'primary', 'all']. All agentwolf agents
    are primary (visible in switcher). This is correct — mode is a category,
    not an agent identifier.
    """
    from agentwolf_server.opencode_server.routes.agent_routes import list_agents
...
async def test_before_consumer_loop_sets_mode_to_agent_name() -> None:
    ...
    server_state.resolve_default_model_info = MagicMock(return_value=("default", "agentwolf"))
