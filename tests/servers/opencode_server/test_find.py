...
class TestFindText:
    ...
    async def test_basic_search(self, async_client: AsyncClient, tmp_project_dir: Path):
        ...
        response = await async_client.get("/find", params={"pattern": "export"})
    ...
    async def test_handles_crlf_line_endings(
        ...
        response = await async_client.get("/find", params={"pattern": "line"})
        assert response.status_code == 200
        matches = response.json()
        assert len(matches) == 3

    async def test_handles_windows_crlf(self, async_client: AsyncClient, tmp_project_dir: Path):
    ...
    async def test_regex_pattern(self, async_client: AsyncClient, tmp_project_dir: Path):
        """Search should support regex patterns."""
        (tmp_project_dir / "code.py").write_text(
            "def hello():\n    pass\ndef world():\n    return 1"
        )
        # Search for function definitions
        response = await async_client.get("/find", params={"pattern": r"def \w+\(\)"})
        assert response.status_code == 200
        matches = response.json()
        assert len(matches) == 2
    ...
    async def test_invalid_regex_returns_400(
        ...
        response = await async_client.get("/find", params={"pattern": "[invalid(regex"})
        assert response.status_code == 400
        assert "regex" in response.json()["detail"].lower()

    async def test_search_in_subdirectories(self, async_client: AsyncClient, tmp_project_dir: Path):
        ...
        response = await async_client.get("/find", params={"pattern": "SEARCH_TARGET"})
        assert response.status_code == 200
        matches = response.json()
        assert len(matches) == 2
    ...
    async def test_returns_line_numbers(self, async_client: AsyncClient, tmp_project_dir: Path):
        """Search results should include correct line numbers."""
        (tmp_project_dir / "lines.txt").write_text("one\ntwo\nthree\nfour\nfive")
        response = await async_client.get("/find", params={"pattern": "three"})
        assert response.status_code == 200
        matches = response.json()
        assert len(matches) == 1
        assert matches[0]["line_number"] == 3

    async def test_limits_results(self, async_client: AsyncClient, tmp_project_dir: Path):
        ...
        response = await async_client.get("/find", params={"pattern": "match"})
        assert response.status_code == 200
        matches = response.json()
...
class TestFindFiles:
    ...
    async def test_find_by_extension(self, async_client: AsyncClient, tmp_project_dir: Path):
    ...
    async def test_find_by_name_pattern(self, async_client: AsyncClient, tmp_project_dir: Path):
        ...
        response = await async_client.get("/find/file", params={"query": "test_*.py"})
        assert response.status_code == 200
        files = response.json()
        assert len(files) == 2
        assert all("test_" in f for f in files)

    async def test_find_in_subdirectories(self, async_client: AsyncClient, tmp_project_dir: Path):
        ...
        response = await async_client.get("/find/file", params={"query": "*.tsx"})
        assert response.status_code == 200
        files = response.json()
        assert len(files) == 3
    ...
    async def test_skips_excluded_directories(
        ...
        response = await async_client.get("/find/file", params={"query": "*.js"})
        assert response.status_code == 200
        files = response.json()
        assert len(files) == 1
    ...
    async def test_returns_relative_paths(self, async_client: AsyncClient, tmp_project_dir: Path):
    ...
    async def test_no_matches_returns_empty(
        ...
        response = await async_client.get("/find/file", params={"query": "*.nonexistent"})
        assert response.status_code == 200
