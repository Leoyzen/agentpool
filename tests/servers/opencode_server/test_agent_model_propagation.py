...
became 'default'/'agentwolf' for the lead agent AND sub-agents.

Root cause: ``_before_consumer_loop`` and ``_reconstruct_tool_parts_from_checkpoint``
hardcoded ``agent_name="agentwolf"``, ``model_id="default"``,
``provider_id="agentwolf"`` when creating the assistant message for the async
...
1. ``_before_consumer_loop`` uses ``agent_name`` from the session state
   (not the hardcoded "agentwolf").
2. ``_before_consumer_loop`` uses model info from pending metadata when
   provided by the REST handler (not the hardcoded "default"/"agentwolf").
3. ``_before_consumer_loop`` falls back to "default"/"agentwolf" only when
   no model info is available.
4. ``_reconstruct_tool_parts_from_checkpoint`` uses ``agent_name`` from the
   session state (not the hardcoded "agentwolf").
"""
...
def _make_integration(
    ...
    if server_state is None:
        ...
        server_state.resolve_default_model_info = Mock(return_value=("default", "agentwolf"))
...
class TestBeforeConsumerLoopAgentName:
    ...
    @pytest.mark.asyncio
    async def test_uses_session_agent_name_not_agentwolf(self) -> None:
        """Assistant message should carry the session's agent_name, not 'agentwolf'."""
        session_pool = Mock()
    ...
    @pytest.mark.asyncio
    async def test_falls_back_to_agentwolf_when_no_session_state(self) -> None:
        """When session state is unavailable, fall back to 'agentwolf' (graceful degradation)."""
        session_pool = Mock()
        session_pool.sessions = Mock()
        ...
        # default agent_name.
        assert ctx.assistant_msg.info.agent == "agentwolf"
...
class TestBeforeConsumerLoopModelInfo:
    ...
    async def test_falls_back_to_default_when_no_pending_model(self) -> None:
        """Without pending model metadata, fall back to 'default'/'agentwolf'."""
        session_pool = Mock()
        session_pool.sessions = Mock()
        session_pool.sessions.get_session = Mock(
            return_value=_make_session_state(agent_name="engineer")
        ...
        assert ctx.assistant_msg.info.provider_id == "agentwolf"
...
class TestReconstructToolPartsAgentName:
    ...
    async def test_uses_session_agent_name(self) -> None:
        ...
        assert assistant_msg.info.agent != "agentwolf"

    @pytest.mark.asyncio
    async def test_falls_back_to_agentwolf_when_no_session_state(self) -> None:
        """When session state is unavailable, fall back to 'agentwolf'."""
        from agentwolf_server.opencode_server.opencode_message_bridge import (
            _reconstruct_tool_parts_from_checkpoint,
        )
        ...
        messages = server_state.messages.get("test-session", [])
        assert len(messages) == 1
        assert messages[0].info.agent == "agentwolf"
