...
async def test_ensure_session_checkpointed_restores_input_provider(
    ...
    with (
        patch.object(mock_state, "broadcast_event", new=AsyncMock()),
        patch(
            "agentwolf_server.opencode_server.input_provider.OpenCodeInputProvider"
        ) as mock_prov_cls,
    ):
        mock_prov = MagicMock()
...
async def test_concurrent_calls_produce_one_session(
    ...
    with (
        patch("agentwolf_server.opencode_server.converters.opencode_to_session_data"),
        patch("agentwolf_server.opencode_server.input_provider.OpenCodeInputProvider"),
...
async def test_store_miss_fallback_creates_and_persists(
    ...
    with (
        patch("agentwolf_server.opencode_server.converters.opencode_to_session_data") as mock_conv,
        patch(
            "agentwolf_server.opencode_server.input_provider.OpenCodeInputProvider"
        ) as mock_prov_cls,
        patch.object(mock_state_store_first, "broadcast_event", new=AsyncMock()),
