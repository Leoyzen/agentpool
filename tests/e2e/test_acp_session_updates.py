...
the agentwolf ACP server. Each skip test documents the test intent so
...
pytestmark = [
    pytest.mark.e2e,
    pytest.mark.skipif(SKIP_NO_BINARY, reason="agentwolf binary not on PATH"),
...
async def _spawn_acp_server(
    ...
    *,
    ...
    """Spawn ``agentwolf serve-acp`` and return a handle with client connection.
...
async def test_user_message_chunk_notification(e2e_config: Path) -> None:
    ...
    Note: The agentwolf ACP server does not currently emit ``user_message_chunk``
