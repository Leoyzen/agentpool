"""L4a smoke E2E test for dynamic team mode via ``agentwolf serve-opencode``.

Spawns a real ``agentwolf serve-opencode`` subprocess with a YAML config
...
async def test_team_mode_server_starts(
    ...
    When: ``agentwolf serve-opencode`` is spawned as a subprocess.
