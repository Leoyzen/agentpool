...
``agentwolf serve-*`` process, waits for it to become healthy, and tears it
...
def _agentwolf_binary_available() -> bool:
    """Check whether the ``agentwolf`` CLI binary is on PATH."""
    return shutil.which("agentwolf") is not None


SKIP_NO_BINARY = not _agentwolf_binary_available()
...
class SubprocessServer:
    """Handle to a spawned ``agentwolf serve-*`` subprocess.
...
def _build_command(
    ...
    *,
    ...
    """
    cmd: list[str] = ["agentwolf", serve_command, str(config_path)]
...
async def subprocess_server(  # noqa: PLR0915
    ...
    """Spawn an ``agentwolf serve-*`` subprocess, reusing cached servers when possible.
...
async def _spawn_server(
    ...
    *,
    ...
    """Spawn an agentwolf server subprocess (internal helper for custom fixtures).
...
async def subprocess_server_with_mcp(
    ...
    """Spawn an ``agentwolf serve-*`` subprocess with the fake MCP server config.
...
class ACPWSServerHandle:
    """Handle to a spawned ``agentwolf serve-acp --transport streamable-http`` server.
...
async def _spawn_acp_ws_server(
    ...
    *,
    ...
    """Spawn ``agentwolf serve-acp --transport streamable-http`` and wait for health.
...
def verify_no_orphaned_processes(
    ...
    if orphans:
        ...
        warnings.warn(
            f"{len(orphans)} orphaned agentwolf subprocess(es) found and killed at session end.",
            stacklevel=2,
        )
