...
async def test_get_agentlet_collects_tool_provider_capabilities(
    ...
    with patch("agentwolf.agents.native_agent.agent.PydanticAgent") as mock_pydantic_agent:
...
async def test_get_agentlet_creates_hooks_capability(
    ...
    with (
        patch("agentwolf.agents.native_agent.agent.PydanticAgent") as mock_pydantic_agent,
        patch(
            "agentwolf.agents.native_agent.tool_intercept.ToolInterceptCapability",
...
async def test_yaml_config_capability_passthrough(mock_agent: Agent[Any]) -> None:
    ...
    from agentwolf_config.capabilities import GenericCapabilityConfig
