...
async def test_acp_turn_prompt_stream_complete_cycle() -> None:
    """Given a mock client, ACPTurn yields RunStarted, PartDelta, StreamComplete."""
...
async def test_acp_turn_prompt_error_yields_run_error_event() -> None:
    """Given a prompt error, ACPTurn yields RunStarted then RunError."""
...
async def test_acp_turn_stream_error_yields_run_error_event() -> None:
    """Given a stream error, ACPTurn yields RunStarted, partial events, then RunError."""
    ...
    # RunStarted, PartDelta (from partial update), RunError
    assert len(events) == 3
    assert isinstance(events[0], RunStartedEvent)
    assert isinstance(events[1], PartDeltaEvent)
    assert isinstance(events[2], RunErrorEvent)
    assert "Stream broken" in events[2].message
...
async def test_acp_turn_empty_prompts_uses_empty_string() -> None:
    ...
    # Should still yield RunStarted and StreamComplete
    assert len(events) == 2
    assert isinstance(events[0], RunStartedEvent)
