...
1. Client connects to agentwolf ACP server (wrapping Claude Code)
2. Lists sessions from Claude Code
3. Loads a session - server delegates to Claude Code's load_session
4. Claude Code replays notifications -> agentwolf forwards -> client receives
...
logging.getLogger("agentwolf").setLevel(logging.DEBUG)
logging.getLogger("agentwolf_server").setLevel(logging.DEBUG)
...
async def main() -> None:
    ...
    # Connect to agentwolf ACP server running Claude Code agent
    print("Starting ACPAgent...")
    async with ACPAgent(
        command="uv",
        args=["run", "agentwolf", "serve-acp", "tests/agents/acp_agent/claude_code_config.yml"],
        name="test_client",
        cwd="/home/phil65/dev/oss/agentwolf",
    ) as agent:
