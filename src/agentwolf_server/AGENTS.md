# agentwolf_server — Protocol Servers
...
- **ACP `session.py`** (905 lines) manages `ACPSession` state including MCP server registration, provider config, mode toggles, and the `CommandStore` for slash commands. It bridges to `agentwolf_commands.base.NodeCommand`.
...
The `UserMessageInsertedEvent` (defined in `agentwolf.agents.events.events`) carries inserted user message content through the EventBus. For ACP, this is handled in two places:
