...
from agentwolf_server.opencode_server.models.session import Session
...
if TYPE_CHECKING:
    from agentwolf.agents.events.events import RunErrorEvent, SpawnSessionStart, StreamCompleteEvent
...
def _reconstruct_tool_parts_from_checkpoint(
    ...
    # session state instead of hardcoding "agentwolf". Falls back to
    # "agentwolf" when the session state is unavailable.
    agent_name = "agentwolf"
    pool = state.pool
