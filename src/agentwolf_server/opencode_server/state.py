...
from agentwolf.diagnostics.lsp_manager import LSPManager
from agentwolf_server.opencode_server.models import SessionStatus
from agentwolf_server.opencode_server.provider_auth import (
    ProviderAuthService,
...
if TYPE_CHECKING:
    ...
    from agentwolf_server.opencode_server.models.question import QuestionToolInfo
...
class ServerState:
    ...
    def parse_model_info(model_name: str | None) -> tuple[str, str]:
        ...
        "agentwolf")`` when the model name is ``None`` or missing a
        provider prefix.
    ...
    def resolve_default_model_info(self) -> tuple[str, str]:
        ...
        3. Falls back to ``("default", "agentwolf")``.
        ...
        if agent_model and self._pool is not None:
            ...
            if manifest and manifest.model_variants:
                from agentwolf_server.shared.model_utils import (
                    _extract_provider,
                    _find_variant_name,
                )
    ...
    def __post_init__(self) -> None:
        ...
        if self.session_controller is not None:
            ...
            if event_bus is not None:
                from agentwolf_server.opencode_server.event_bridge import (
                    OpenCodeEventBridge,
                )
    ...
    async def mark_session_idle(self, session_id: str) -> None:
        ...
        from agentwolf_server.opencode_server.models import SessionIdleEvent, SessionStatusEvent
        from agentwolf_server.opencode_server.session_pool_integration import set_session_status
    ...
    async def emit_session_turn_complete(self, session_id: str) -> None:
        ...
        from agentwolf_server.opencode_server.models import SessionIdleEvent
