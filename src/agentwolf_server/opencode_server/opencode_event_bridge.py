...
from agentwolf_server.opencode_server.event_adapter import OpenCodeEventAdapter
...
from agentwolf_server.opencode_server.opencode_session_routes import (
    ensure_session,
    set_session_status,
)
...
class OpenCodeEventBridgeMixin:
    ...
    async def _ensure_child_session_visible(
        ...
        if title is not None and session.title != title:
            session.title = title
            from agentwolf_server.opencode_server.models import SessionUpdatedEvent
