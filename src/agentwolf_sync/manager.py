...
from agentwolf_sync.git import GitRepo
from agentwolf_sync.models import FileChange
from agentwolf_sync.packages import PackageRegistry
from agentwolf_sync.parsers import BUILTIN_PARSERS
from agentwolf_sync.resources import ResourceRegistry


if TYPE_CHECKING:
    from collections.abc import Awaitable, Callable

    from agentwolf_sync.models import SyncMetadata
    from agentwolf_sync.packages import PackageChange
    from agentwolf_sync.parsers import MetadataParser
...
class SyncManager:
    ...
    def __init__(
        ...
        Args:
            ...
            registry_file: Path to resource registry (default: .agentwolf/resources.yml)
            packages_file: Path to packages registry (default: .agentwolf/packages.yml)
        """
        self.root = Path(root).resolve()
        self.git = GitRepo(self.root)

        registry_path = (
            Path(registry_file) if registry_file else self.root / ".agentwolf" / "resources.yml"
        )
        self.resources = ResourceRegistry(registry_file=registry_path)

        packages_path = (
            Path(packages_file) if packages_file else self.root / ".agentwolf" / "packages.yml"
        )
