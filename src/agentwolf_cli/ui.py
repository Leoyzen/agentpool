...
def opencode_ui_command(
    ...
    Examples:
        # Start server + TUI
        agentwolf ui opencode

        # Use specific config and agent
        agentwolf ui opencode agents.yml --agent myagent

        # Custom port
        agentwolf ui opencode --port 8080

        # Attach to existing server (no server startup)
        agentwolf ui opencode --attach
        agentwolf ui opencode --attach --port 8080
    """
    ...
    server_cmd = ["agentwolf", "serve-opencode", "--host", host, "--port", str(port)]
    if config:
...
def toad_ui_command(
    ...
    By default uses stdio transport where Toad spawns the agentwolf server.
    ...
    Examples:
        ...
        # Use specific config
        agentwolf ui toad agents.yml

        # WebSocket transport
        agentwolf ui toad --websocket

        # WebSocket with custom port
        agentwolf ui toad --websocket --port 9000
    """
    if websocket:
...
def _run_toad_stdio(config: str | None) -> None:
    ...
    # Build agentwolf command that Toad will spawn
    agentwolf_cmd = "agentwolf serve-acp"
    if config:
        agentwolf_cmd += f" {config}"
    os.system("clear" if os.name != "nt" else "cls")
    # Run toad with agentwolf as subprocess
    result = subprocess.run(
        ["uvx", "--from", "batrachian-toad@latest", "toad", "acp", agentwolf_cmd],
        check=False,
    )
    if result.returncode not in {0, 130}:  # 130 = Ctrl+C
...
def opencode_desktop_command(  # noqa: PLR0915
    ...
    Examples:
        ...
        agentwolf ui desktop

        # Use specific config and agent
        agentwolf ui desktop agents.yml --agent myagent

        # Custom port
        agentwolf ui desktop --port 8080

        # Configure desktop to attach to existing server
        agentwolf ui desktop --attach
        agentwolf ui desktop --attach --port 8080

        # After using --attach, reset to default (spawn local server)
        agentwolf ui desktop --attach --port 0  # port 0 clears the setting
    """
    url = f"http://{host}:{port}"
