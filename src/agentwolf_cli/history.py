...
from agentwolf_cli import log, resolve_agent_config
from agentwolf_cli.cli_types import GroupBy
from agentwolf_cli.common import OutputFormat, format_output, output_format_opt
...
def show_history(
    ...
    Examples:
        ...
        agentwolf history show -n 5

        # Show conversations from last 24 hours
        agentwolf history show --period 24h

        # Show conversations since specific date
        agentwolf history show --since 2024-01-01

        # Search for specific content
        agentwolf history show --query "database schema"

        # Show GPT-5 conversations with token usage
        agentwolf history show --model gpt-5 --tokens

        # Compact view of recent conversations
        agentwolf history show --period 1d --compact
    """
    import anyio
...
def show_stats(
    ...
    Examples:
        ...
        agentwolf history stats

        # Show daily stats for specific agent
        agentwolf history stats myagent --group-by day

        # Show model usage for last week
        agentwolf history stats --period 1w --group-by model
    """
    import anyio

    from agentwolf.utils.parse_time import parse_time_period
    from agentwolf_storage.formatters import format_stats
    from agentwolf_storage.models import StatsFilters
...
def reset_history(
    ...
    Examples:
        ...
        agentwolf history reset

        # Clear without confirmation
        agentwolf history reset --confirm

        # Clear history for specific agent
        agentwolf history reset --agent myagent

        # Drop and recreate tables (for schema changes)
        agentwolf history reset --hard --confirm
    """
    import anyio
