...
def acp_command(  # noqa: PLR0915
    ...
    1. Global config - ~/.config/agentwolf/agentwolf.yml for user preferences
    2. Custom config - AGENTPOOL_CONFIG env var for CI/deployment overrides
    3. Project config - agentwolf.yml in project/git root for project-specific settings
    ...
    Transport Options:
        ...
        agentwolf serve-acp agents.yml --transport streamable-http --port 8080

        # Legacy WebSocket transport (deprecated)
        agentwolf serve-acp agents.yml --transport websocket --ws-port 8765
    """
    from acp import ACPWebSocketTransport, StdioTransport, WebSocketTransport
    from agentwolf import log
    from agentwolf.config_resources import ACP_ASSISTANT
    ...
    from agentwolf_server.acp_server import ACPServer
    ...
    log_dir = user_log_path("agentwolf", appauthor=False)
    ...
    if mcp_config:
        from agentwolf_config.mcp_server import parse_mcp_servers_json
