...
1. Global config - ~/.config/agentwolf/agentwolf.yml - user preferences
...
def get_global_config_dir() -> Path:
    ...
    Returns:
        Path to ~/.config/agentwolf (or platform equivalent)
    """
    return Path(platformdirs.user_config_dir("agentwolf", appauthor=False))
...
def find_project_config(start_dir: JoinablePathLike | None = None) -> Path | None:
    ...
    Searches for agentwolf.yml/yaml/json in the current directory and parent
