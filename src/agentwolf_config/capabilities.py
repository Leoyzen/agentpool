...
2. **Entry-point names** (e.g. ``mermaid_lint``) resolved via the
   ``agentwolf.capabilities`` entry-point group using
...
IMPORT_MAP: dict[str, str] = {
    "loop_detection": "agentwolf.capabilities.loop_detection.LoopDetectionCapability",
    "token_budget": "agentwolf.capabilities.token_budget.TokenBudgetCapability",
    "tool_output_budget": ("agentwolf.capabilities.tool_output_budget.ToolOutputBudgetCapability"),
    "dcp": "agentwolf.capabilities.dcp.DynamicContextPruningCapability",
    "skill_activation": "agentwolf.capabilities.skill_manager_cap:SkillManagerCap",
    "memory": "agentwolf.capabilities.memory.MemoryCapability",
    "modality_filter": "agentwolf.capabilities.modality_filter.ModalityFilterCapability",
}
...
class EntryPointCapabilityConfig(BaseModel):
    ...
    ``agentwolf.capabilities`` entry-point group (e.g. ``"mermaid_lint"``).
    ...
    type: str
    """Entry-point name registered under ``agentwolf.capabilities``."""
    ...
    def build(self) -> Any:
        ...
        in the ``agentwolf.capabilities`` group, avoiding a dependency
        on the :mod:`agentwolf` package (which would violate the
        import-linter contract that ``agentwolf_config`` must not import
        from ``agentwolf``).
        ...
        eps = entry_points(group="agentwolf.capabilities")
        for ep in eps:
