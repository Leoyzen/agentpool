...
if TYPE_CHECKING:
    ...
    from agentwolf.prompts.prompts import BasePrompt  # noqa: TC004
...
class Job[TDeps, TResult = str](Schema):
    ...
    prompt: str | ImportString[str] | BasePrompt = Field(
        examples=["Analyze the following code", "agentwolf.prompts.prompts:BasePrompt"],
        title="Task prompt",
    )
    ...
    required_dependency: ImportString[type[TDeps]] | None = Field(
        default=None,
        validate_default=True,
        examples=["agentwolf.agents.context:AgentContext"],
    ...
    async def can_be_executed_by(self, agent: Agent[Any, Any]) -> bool:
        ...
        if self.requires_vision:
            from agentwolf.utils.model_capabilities import supports_vision
