...
class MCPConnectionPool:
    ...
    def __init__(
        ...
        *,
        ...
        self._aggregating_provider = AggregatingResourceProvider(
            providers=[], name="mcp_pool"
        )
        ...
        async with self._lock:
            ...
            if len(self._connections) >= self._max_processes:
                ...
                if recycled is None:
                    ...
                    raise RuntimeError(
                        f"MCP connection pool at capacity ({self._max_processes})"
                    )
            ...
            try:
                ...
                logger.exception(
                    "Failed to spawn MCP subprocess for '%s'", key
                )
                raise
    ...
    def release_connection(self, server_config: MCPServerConfig) -> None:
        ...
        if conn is None:
            logger.debug(
                "release_connection called for unknown config", client_id=key
            )
            return
    ...
    async def shutdown(self) -> None:
        ...
        async with self._lock:
            providers_to_close = [
                conn.provider for conn in self._connections.values()
            ]
        ...
        try:
            with anyio.CancelScope(shield=True):
                try:
                    ...
                    logger.warning(
                        "MCP connection pool shutdown timed out after 5s"
                    )
        except Exception:
    ...
    async def _cleanup_idle(self) -> None:
        ...
        for key, provider in to_close:
            ...
            try:
                ...
                logger.exception(
                    "Error closing idle MCP provider", client_id=key
                )
