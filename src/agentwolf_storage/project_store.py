...
# Config file names to search for in .agentwolf/ directory
...
def resolve_config(project: ProjectData | None = None, cwd: str | None = None) -> str | None:
    ...
    2. .agentwolf/config.yml in project worktree (auto-discovered)
...
def generate_project_id(worktree: str) -> str:
    ...
    !!! note "Dual ID scheme"
        ...
        ``agentwolf_storage.opencode_provider.helpers``, which derives the ID
...
def discover_config_path(worktree: str) -> str | None:
    """Search for config file in .agentwolf/ directory.
    ...
    """
    config_dir = Path(worktree) / ".agentwolf"
...
class ProjectStore:
    ...
    async def refresh_config(self, project_id: str) -> ProjectData | None:
        ...
        Useful after user adds .agentwolf/config.yml to their project.
