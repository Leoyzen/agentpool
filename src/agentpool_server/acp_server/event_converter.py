...
class ACPEventConverter:
    ...
    """Maps child_session_id → tool_call_id for subagent tracking."""

    _foreground_children: set[str] = field(default_factory=set)
    """Set of child_session_ids running in foreground mode."""
    ...
    def reset(self) -> None:
        ...
        self._foreground_children.clear()
        self._current_message_id = str(uuid.uuid4())
        self.last_usage = None

    def cleanup(self) -> None:
        """Clean up all tracked subagent state.

        Should be called when the parent session is closed to ensure
        no memory leaks from orphaned subagent tracking state.
        """
        self._subagent_tool_map.clear()
        self._foreground_children.clear()
    ...
    async def convert(  # noqa: PLR0915
        ...
        match event:
            ...
            case SpawnSessionStart(
                ...
                yield ToolCallStart(
                    tool_call_id=tc_id or f"subagent:{child_id}",
                    title=f"{'⚡' if mechanism == 'spawn' else '🚀'} {source_name}",
                    kind="subagent",
                    subagent=SubagentRunInfo(
                        subagent_id=source_name,
                        name=source_name,
                        description=description,
                        depth=depth,
                        status="running",
                ...
                self._subagent_tool_map[child_id] = tc_id or f"subagent:{child_id}"
                if run_mode == "foreground":
                    self._foreground_children.add(child_id)
            ...
            case SubAgentEvent(
                ...
                tc_id = self._subagent_tool_map.get(child_id)
                if tc_id:
                    yield ToolCallProgress(
                        tool_call_id=tc_id,
                        status="completed",
                ...
                self._foreground_children.discard(child_id)
                self._subagent_tool_map.pop(child_id, None)

            case SubAgentEvent(
                child_session_id=child_id,
                source_name=source_name,
                event=RunErrorEvent(message=msg),
            ) if child_id is not None:
                tc_id = self._subagent_tool_map.get(child_id)
                if tc_id:
                    yield ToolCallProgress(
                        tool_call_id=tc_id,
                        status="failed",
                        content=[ContentToolCallContent.text(f"Error: {msg}")],
                        subagent=SubagentRunInfo(
                            subagent_id=source_name,
                            name=source_name,
                            child_session_id=child_id,
