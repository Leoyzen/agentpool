...
def create_app(*, agent: BaseAgent[Any, Any], working_dir: str | None = None) -> FastAPI:  # noqa: PLR0915
    ...
    async def on_todo_change(tracker: TodoTracker) -> None:
        ...
        todos = [
            Todo(content=e.content, status=e.status, priority=e.priority) for e in tracker.entries
        ]
    ...
    async def lifespan(app: FastAPI) -> AsyncIterator[None]:
        nonlocal branch_watcher, file_watcher
        ...
        file_watcher = FileWatcher(paths=[state.working_dir], callback=on_file_change, debounce=500)
        ...
        async def check_for_updates() -> None:
            ...
            if latest and compare_versions(__version__, latest):
                logger.info("Update available", current_version=__version__, latest=latest)
                event = TuiToastShowEvent.create(
                    title="Update Available",
                    message=f"agentpool {latest} is available (current: {__version__})",
                    variant="info",
                    duration=10000,
                )
                await state.broadcast_event(event)
