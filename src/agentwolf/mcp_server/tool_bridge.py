...
from agentwolf.utils.signatures import filter_schema_params, get_params_matching_predicate
...
if TYPE_CHECKING:
    ...
    from agentwolf.agents import AgentContext
...
def _convert_to_tool_result(result: Any) -> FastMCPToolResult:
    ...
    from agentwolf.tools.base import ToolResult as AgentPoolToolResult
...
class ToolManagerBridge:
    ...
    def resolved_server_name(self) -> str:
        """Get the server name."""
        return self.server_name or f"agentwolf-{self.node.name}-tools"
    ...
    def _register_single_tool(self, tool: Tool) -> None:
        ...
        class _BridgeTool(FastMCPTool):
            """Custom FastMCP Tool that wraps a agentwolf Tool.
    ...
    async def invoke_tool_with_context(
        ...
        """
        from agentwolf.tasks import ToolSkippedError
