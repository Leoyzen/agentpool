...
This module provides :class:`ACPTurn`, a :class:`~agentwolf.orchestrator.turn.Turn`
subclass that drives an ACP client through a single prompt → stream → complete
cycle, yielding :class:`~agentwolf.agents.events.RichAgentStreamEvent` items
...
def _convert_updates_to_model_messages(
    ...
    *,
    ...
    Uses :class:`~agentwolf.agents.acp_agent.acp_converters.ACPMessageAccumulator`
    to build :class:`~agentwolf.messaging.ChatMessage` objects from the raw
    ...
    """
    from agentwolf.agents.acp_agent.acp_converters import ACPMessageAccumulator
