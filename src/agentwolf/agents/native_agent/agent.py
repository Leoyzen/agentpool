...
from agentwolf.agents.exceptions import UnknownCategoryError, UnknownModeError
...
if TYPE_CHECKING:
    ...
    from agentwolf.agents.modes import ModeCategory
    from agentwolf.common_types import (
        AnyEventHandlerType,
        EndStrategy,
        ModelType,
        ProcessorCallback,
    ...
    from agentwolf.mcp_server.config_snapshot import McpConfigEntry
    from agentwolf.messaging import MessageNode
    from agentwolf.models.agents import NativeAgentConfig, ToolMode
    from agentwolf.models.model_configs import BaseModelConfig
    from agentwolf.orchestrator.turn import Turn
    from agentwolf.prompts.prompts import PromptType
    ...
    from agentwolf_config.nodes import ToolConfirmationMode
...
def _build_capability_from_config(config: Any) -> Any:
    ...
    Delegates to ``agentwolf_config.capabilities.build_capability()``,
    using late import to avoid a static config→core dependency.
    """
    from agentwolf_config.capabilities import build_capability
...
class Agent[TDeps = None, OutputDataT = str](BaseAgent[TDeps, OutputDataT]):
    ...
    def __init__(  # noqa: PLR0915
        ...
        *,
        ...
        from agentwolf.prompts.conversion_manager import ConversionManager
        from agentwolf_commands.pool import CompactCommand
        ...
        if self.config and self.config.capabilities:
            ...
            from agentwolf_config.capabilities import (
    ...
    def _resolve_history_processors(self, *, _warn: bool = True) -> list[Callable[..., Any]]:
        ...
        if memory_cfg := self.conversation._config:
            ...
            if processor_paths:
                from agentwolf.utils.importing import import_callable
    ...
    def from_config(  # noqa: PLR0915
        ...
        *,
        ...
        from agentwolf.models.manifest import AgentsManifest
        from agentwolf.utils.result_utils import to_type
        from agentwolf_config.system_prompts import (
            FilePromptConfig,
        ...
        from agentwolf.models.model_configs import BaseModelConfig, StringModelConfig
    ...
    def from_callback(
        ...
        **kwargs: Any,
        ...
        from agentwolf.utils.signatures import get_return_type
    ...
    def name(self) -> str:
        """Get agent name."""
        return self._name or "agentwolf"
    ...
    def _resolve_model_string(self, model: str) -> tuple[Model, ModelSettings | None]:
        ...
        """
        from agentwolf.utils.model_helpers import infer_model
    ...
    async def _resolve_model_capabilities(
        ...
        if len(model_names) == 1:
            from agentwolf.host.stubs import resolve_capabilities
    ...
    async def get_agentlet[AgentOutputType](  # noqa: PLR0915
            ...
            if isinstance(provider, _AbstractCapability):
                ...
                try:
                    ...
                    for tool in provider_tools:
                        from agentwolf.agents.native_agent.tool_wrapping import wrap_tool
        ...
        from agentwolf.agents.native_agent.tool_intercept import ToolInterceptCapability
        ...
        from agentwolf.agents.native_agent.deferred_bridge import (
        ...
        from agentwolf.agents.native_agent.approval_bridge import (
            ...
            if registry is not None:
                ...
                if session_id not in self._registered_session_ids:
                    ...
                    if pool is not None:
                        ...
                        if pool_caps:
                            from agentwolf.capabilities.skill_manager_cap import (
                                SkillManagerCap,
                            )
        ...
        if resolved_caps is not None:
            ...
            from agentwolf.capabilities.modality_filter import (
    ...
    async def _execute_node(self, *prompts: Any, **kwargs: Any) -> ChatMessage[Any]:
        ...
        :class:`~agentwolf.messaging.graph_adapter.MessageNodeStep`) and
        delegates execution to :class:`~agentwolf.agents.native_agent.turn.NativeTurn`,
        ...
        Args:
            ...
            **kwargs: Must contain ``_state`` (an
                :class:`~agentwolf.messaging.graph_adapter.AgentPoolState`).
        ...
        """
        from agentwolf.messaging.graph_adapter import AgentPoolState
    ...
    async def _stream_events(
        ...
        **pydantic_ai_kwargs: Any,
        ...
        Delegates to :class:`~agentwolf.agents.native_agent.turn.NativeTurn`
        ...
        from agentwolf.orchestrator.run import inject_cancelled_tool_results
    ...
    async def get_modes(self) -> list[ModeCategory]:
        ...
        from agentwolf.agents.modes import ModeCategory as ModeCategoryRuntime, ModeInfo
        from agentwolf.agents.native_agent.helpers import (
            get_model_category,
