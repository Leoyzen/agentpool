...
| LifecycleConfig Pydantic model | `agentwolf_config/lifecycle.py` |
...
| Skill config models (McpServerConfig, ToolConfig) | `agentwolf_config/skills.py` |
...
- **Config model imports from core**: Import config types from `agentwolf_config.*`, not `agentwolf.models`, to avoid circular deps.
...
- **Mixing `lifecycle.EventEnvelope` with `orchestrator.event_bus.EventEnvelope`**: These are separate types with different roles. Import from `agentwolf.lifecycle.types` for lifecycle transport envelopes; from `agentwolf.orchestrator.event_bus` for internal EventBus envelopes.
...
- **Config model lives in separate package**: `agentwolf_config/` exists solely to prevent import cycles with protocol servers.
