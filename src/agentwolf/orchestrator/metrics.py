...
class SessionPoolMetrics:
    ...
    def to_prometheus(self) -> str:
        ...
        All metric names use the ``agentwolf_`` prefix and the same
        ...
        lines.append("# TYPE agentwolf_sessions_total gauge")
        lines.append(f"agentwolf_sessions_total {self.active_sessions}")

        lines.append("# TYPE agentwolf_active_turns_total gauge")
        lines.append(f"agentwolf_active_turns_total {self.active_turns}")

        lines.append("# TYPE agentwolf_auto_resume_total counter")
        lines.append(f"agentwolf_auto_resume_total {self.auto_resume_count}")

        lines.append("# TYPE agentwolf_event_bus_subscribers gauge")
        for session_id, count in self.event_bus_queue_depth.items():
            sid = session_id.replace('"', '\\"')
            lines.append(f'agentwolf_event_bus_subscribers{{session_id="{sid}"}} {count}')

        lines.append("# TYPE agentwolf_session_lifetime_seconds gauge")
        lines.append(f"agentwolf_session_lifetime_seconds {self.session_lifetime_seconds:.3f}")

        lines.append("# TYPE agentwolf_turn_latency_ms summary")
        lines.append(f'agentwolf_turn_latency_ms{{quantile="0.99"}} {self.turn_latency_p99:.3f}')

        lines.append("# TYPE agentwolf_active_runs_by_agent_type gauge")
        for agent_type, count in self.active_runs_by_agent_type.items():
            at = agent_type.replace('"', '\\"')
            lines.append(f'agentwolf_active_runs_by_agent_type{{agent_type="{at}"}} {count}')
