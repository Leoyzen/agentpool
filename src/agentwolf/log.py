"""Logging configuration for agentwolf with structlog support."""
...
def get_logger(name: str, log_level: LogLevel | None = None) -> structlog.stdlib.BoundLogger:
    ...
    Args:
        name: The name of the logger, will be prefixed with 'agentwolf.'
    ...
    logger = structlog.get_logger(f"agentwolf.{name}")
    if log_level is not None:
        if isinstance(log_level, str):
            log_level = getattr(logging, log_level.upper())
            assert log_level
        # Set level on underlying stdlib logger
        stdlib_logger = logging.getLogger(f"agentwolf.{name}")
