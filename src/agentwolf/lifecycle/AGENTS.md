...
- **Mixing `lifecycle.EventEnvelope` with `orchestrator.event_bus.EventEnvelope`**: Import from `agentwolf.lifecycle.types` for lifecycle transport; from `agentwolf.orchestrator.event_bus` for internal EventBus.
