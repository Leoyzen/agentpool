...
:class:`~agentwolf.capabilities.skill_activation.SkillActivationCapability`
with a single capability that:

- Holds local skills as ``dict[str, Skill]`` (no per-skill capability wrappers).
- Queries child :class:`~agentwolf.capabilities.mcp_server_cap.McpServerCap`
...
- Imports per-skill Python tools via :class:`~agentwolf.skills.skill_tool_manager.SkillToolManager`.
- Creates per-skill :class:`~agentwolf.capabilities.mcp_server_cap.McpServerCap` instances.
- Applies ``allowed_tools`` filtering via ``get_wrapper_toolset()``.
- Inherits change stream merging and lifecycle from
  :class:`~agentwolf.capabilities.combined_toolset.CombinedToolsetCapability`.
"""
