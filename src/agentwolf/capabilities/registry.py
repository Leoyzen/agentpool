...
capability classes registered via the ``agentwolf.capabilities`` entry-point
group. External packages can register new capability types by adding
an entry to their ``pyproject.toml``:

```toml
[project.entry-points."agentwolf.capabilities"]
...
_ENTRY_POINT_GROUP = "agentwolf.capabilities"
...
def discover_entry_point_capabilities() -> dict[str, type[AbstractCapability[object]]]:
    ...
    Loads all entry points in the ``agentwolf.capabilities`` group and
