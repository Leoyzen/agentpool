...
All Prometheus metric names use the `agentwolf_` prefix and identical label names to the pre-SessionPool Grafana dashboards. **No dashboard changes are required.**

| Metric | Type | Description |
|--------|------|-------------|
| `agentwolf_sessions_total` | gauge | Active sessions |
| `agentwolf_active_turns_total` | gauge | Turns in progress |
| `agentwolf_auto_resume_total` | counter | Auto-resume iterations |
| `agentwolf_event_bus_subscribers` | gauge | Subscribers per session (`session_id` label) |
| `agentwolf_session_lifetime_seconds` | gauge | Average closed-session lifetime |
| `agentwolf_turn_latency_ms` | summary | Turn latency p99 (`quantile="0.99"`) |
...
```python
from agentwolf.orchestrator import SessionPool
...
- `agentwolf_event_bus_subscribers` gauge will show high counts for specific sessions.
...
```text
# TYPE agentwolf_sessions_total gauge
agentwolf_sessions_total 42
# TYPE agentwolf_active_turns_total gauge
agentwolf_active_turns_total 3
# TYPE agentwolf_auto_resume_total counter
agentwolf_auto_resume_total 17
# TYPE agentwolf_event_bus_subscribers gauge
agentwolf_event_bus_subscribers{session_id="sess_abc"} 2
# TYPE agentwolf_session_lifetime_seconds gauge
agentwolf_session_lifetime_seconds 1245.300
# TYPE agentwolf_turn_latency_ms summary
agentwolf_turn_latency_ms{quantile="0.99"} 150.000
```
